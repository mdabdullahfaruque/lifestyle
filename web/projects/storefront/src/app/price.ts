 ...
 * parses it once, at the edge, and never does arithmetic on the result beyond the saving percentage
 * — which is display-only and rounded down, so a "SAVE 16%" badge can never overstate the discount.
 */
...
/** `1890.00` → `৳1,890` — whole taka, because sub-unit precision is noise at these prices. */
...
export function priceParts(price: MoneyRange, compareAt?: string | null): PriceParts {
  const now = formatMoney(price.min, price.currency);
  ...
  return {
    ...
    was: hasSaving ? formatMoney(String(previous), price.currency) : null,
